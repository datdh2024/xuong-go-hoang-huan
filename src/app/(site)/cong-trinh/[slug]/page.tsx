...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const project = await client.fetch<ProjectData>(projectBySlugQuery, { slug });
  if (!project) return {};
  return {
    title: project.title,
...
export async function generateStaticParams() {
  const projects = await client.fetch<Array<{ slug: { current: string } }>>(allProjectsQuery);
      ...
      <div className="relative h-72 md:h-[500px]">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-8 w-full">
            <Link
              href="/cong-trinh"
              className="inline-flex items-center gap-1 text-wood-200 hover:text-gold-400 text-sm mb-4 transition-colors"
            >
              <ArrowLeft size={14} /> Tất cả công trình
            </Link>
            <h1 className="font-cormorant text-4xl md:text-5xl font-bold text-white italic">
              {project.title}
            </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2">
            <h2 className="font-cormorant text-3xl font-semibold text-wood-700 mb-4">
              Mô tả công trình
            </h2>
            ...
            {galleryImages.length > 0 && (
              <>
                <h3 className="font-cormorant text-2xl font-semibold text-wood-700 mb-4">
                  Thư viện ảnh
                </h3>
          ...
          <div>
            <div className="bg-wood-50 border border-wood-200 rounded-lg p-6 mb-6 sticky top-28">
              <h3 className="font-cormorant text-xl font-semibold text-wood-700 mb-4">
                Thông tin dự án
              </h3>
              <dl className="space-y-3 text-sm">
                {[
                  { label: "Loại công trình", value: project.category?.name },
                  { label: "Địa điểm", value: project.location },
                  { label: "Năm hoàn thành", value: project.completedYear },
                ].map((item) => (
                  <div
                    key={item.label}
                    className="flex justify-between border-b border-wood-200 pb-2"
                  >
